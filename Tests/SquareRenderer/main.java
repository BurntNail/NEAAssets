...
public class main {
    ...
    public main ()
    {
        ...
        wh = new Dimension(TILE_SIZE * w, TILE_SIZE * w);
    ...
    public static void main(String[] args) {
        ...
        while (true)
        {
            if(System.currentTimeMillis() - current > msPerUpdate)
            {
                ...
                for (int x = 0; x < w; x++) {
                    for (int y = 0; y < w; y++) {
                        int rnd = r.nextInt(posibbleFiles.length);
                        String f = posibbleFiles[rnd];
    ...
    private static String[][] getImgs (int w ) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < w; y++) {
                int index = rnd.nextInt(posibbleFiles.length);
                files[x][y] = posibbleFiles[index];
            }
        }
